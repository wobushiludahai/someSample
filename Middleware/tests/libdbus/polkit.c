...
static void add_dict_with_string_value(DBusMessageIter *iter, const char *key, const char *str)
{
...
static void add_empty_string_dict(DBusMessageIter *iter)
{
    ...
    dbus_message_iter_open_container(iter, DBUS_TYPE_ARRAY,
        DBUS_DICT_ENTRY_BEGIN_CHAR_AS_STRING DBUS_TYPE_STRING_AS_STRING DBUS_TYPE_STRING_AS_STRING
            DBUS_DICT_ENTRY_END_CHAR_AS_STRING,
        &dict);
...
static void add_arguments(DBusConnection *conn, DBusMessageIter *iter, const char *action, dbus_uint32_t flags)
{
    ...
    void (*function)(dbus_bool_t authorized, void *user_data), void *user_data, int timeout)
{
    ...
    if (dbus_connection_send_with_reply(conn, msg, &call, timeout) == FALSE)
    {
    ...
    dbus_pending_call_set_notify(call, authorization_reply, data, dbus_free);
