...
static struct filter_data *filter_data_find_match(DBusConnection *connection, const char *name, const char *owner,
...
static void format_rule(struct filter_data *data, char *rule, size_t size)
{
    ...
    if (sender)
        offset += snprintf(rule + offset, size - offset, ",sender='%s'", sender);
    if (data->path)
        offset += snprintf(rule + offset, size - offset, ",path='%s'", data->path);
    if (data->interface)
        offset += snprintf(rule + offset, size - offset, ",interface='%s'", data->interface);
    if (data->member)
        offset += snprintf(rule + offset, size - offset, ",member='%s'", data->member);
    if (data->argument)
        snprintf(rule + offset, size - offset, ",arg0='%s'", data->argument);
}

static gboolean add_match(struct filter_data *data, DBusHandleMessageFunction filter)
{
    DBusError err;
    ...
    {
        error("Adding match rule \"%s\" failed: %s", rule, err.message);
        dbus_error_free(&err);
        return FALSE;
    }
...
static gboolean remove_match(struct filter_data *data)
{
    ...
    if (dbus_error_is_set(&err))
    {
        error("Removing owner match rule for %s failed: %s", rule, err.message);
        dbus_error_free(&err);
        return FALSE;
    }
...
static void filter_data_free(struct filter_data *data)
{
    ...
    if (filter_data_find(data->connection) == NULL)
        dbus_connection_remove_filter(data->connection, message_filter, NULL);
...
static struct filter_data *filter_data_get(DBusConnection *connection, DBusHandleMessageFunction filter,
    const char *sender, const char *path, const char *interface, const char *member, const char *argument)
{
...
proceed:
    data = filter_data_find_match(connection, name, owner, path, interface, member, argument);
    if (data)
        return data;
...
static struct filter_callback *filter_data_find_callback(struct filter_data *data, guint id)
{
    GSList *l;
...
static gboolean filter_data_remove_callback(struct filter_data *data, struct filter_callback *cb)
{
...
static DBusHandlerResult service_filter(DBusConnection *connection, DBusMessage *message, void *user_data)
{
    struct filter_data *data = user_data;
    struct filter_callback *cb;
    char *name, *old, *new;
    ...
    if (!dbus_message_get_args(
            message, NULL, DBUS_TYPE_STRING, &name, DBUS_TYPE_STRING, &old, DBUS_TYPE_STRING, &new, DBUS_TYPE_INVALID))
    {
...
static DBusHandlerResult message_filter(DBusConnection *connection, DBusMessage *message, void *user_data)
{
    ...
    for (current = listeners; current != NULL; current = current->next)
    {
        ...
        if (data->interface && g_str_equal(iface, data->interface) == FALSE)
            continue;

        if (data->member && g_str_equal(member, data->member) == FALSE)
            continue;

        if (data->argument && g_str_equal(arg, data->argument) == FALSE)
            continue;
        ...
        if (!data->callbacks)
            delete_listener = g_slist_prepend(delete_listener, current);
    }

    if (delete_listener == NULL)
        return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;

    for (current = delete_listener; current != NULL; current = delete_listener->next)
    {
...
static void service_reply(DBusPendingCall *call, void *user_data)
{
    ...
    if (dbus_message_get_args(reply, &err, DBUS_TYPE_STRING, &data->owner, DBUS_TYPE_INVALID) == FALSE)
        goto fail;
...
static void check_service(DBusConnection *connection, const char *name, struct filter_callback *callback)
{
    DBusMessage *message;
    struct service_data *data;
    ...
    if (dbus_connection_send_with_reply(connection, message, &data->call, -1) == FALSE)
    {
    ...
    GDBusWatchFunction disconnect, void *user_data, GDBusDestroyFunction destroy)
{
    ...
    data = filter_data_get(
        connection, service_filter, DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS, "NameOwnerChanged", name);
    if (data == NULL)
        return 0;

    cb = filter_data_add_callback(data, connect, disconnect, NULL, destroy, user_data);
    if (cb == NULL)
        return 0;
    ...
    const char *member, GDBusSignalFunction function, void *user_data, GDBusDestroyFunction destroy)
{
    ...
    data = filter_data_get(connection, signal_filter, sender, path, interface, member, NULL);
    if (data == NULL)
        return 0;

    cb = filter_data_add_callback(data, NULL, NULL, function, destroy, user_data);
    if (cb == NULL)
        return 0;
    ...
    const char *interface, GDBusSignalFunction function, void *user_data, GDBusDestroyFunction destroy)
{
    ...
    data = filter_data_get(
        connection, signal_filter, sender, path, DBUS_INTERFACE_PROPERTIES, "PropertiesChanged", interface);
    if (data == NULL)
        return 0;
