...
static gboolean disconnected_signal(DBusConnection *conn, DBusMessage *msg, void *data)
{
...
static inline void queue_dispatch(DBusConnection *conn, DBusDispatchStatus status)
{
...
static void dispatch_status(DBusConnection *conn, DBusDispatchStatus status, void *data)
{
...
{
    dbus_connection_set_watch_functions(conn, add_watch, remove_watch, watch_toggled, conn, NULL);

    dbus_connection_set_timeout_functions(conn, add_timeout, remove_timeout, timeout_toggled, NULL, NULL);

    dbus_connection_set_dispatch_status_function(conn, dispatch_status, NULL, NULL);
}

static gboolean setup_bus(DBusConnection *conn, const char *name, DBusError *error)
{
    gboolean result;
    DBusDispatchStatus status;
...
gboolean rpc_request_name(DBusConnection *connection, const char *name, DBusError *error)
{
    ...
    result = dbus_bus_request_name(connection, name, DBUS_NAME_FLAG_DO_NOT_QUEUE, error);
