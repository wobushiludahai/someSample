...
enum
{
  _MY_INTERFACE_BAR_SIGNAL,
};

static unsigned _MY_INTERFACE_SIGNALS[1] = { 0 };
...
inline static void
my_interface_signal_marshal_bar_signal (
    GClosure     *closure,
    GValue       *return_value,
    unsigned int  n_param_values,
    const GValue *param_values,
...
inline static void
my_interface_method_marshal_my_method (
    GClosure     *closure,
    GValue       *return_value,
    unsigned int  n_param_values,
    const GValue *param_values,
...
my_interface_default_init (MyInterfaceIface *iface)
{
   ...
   */
  _MY_INTERFACE_SIGNALS[_MY_INTERFACE_BAR_SIGNAL] =
...
my_interface_emit_bar_signal (
    ...
    const gchar *arg_boo)
{
  g_signal_emit (object, _MY_INTERFACE_SIGNALS[_MY_INTERFACE_BAR_SIGNAL], 0, arg_blah, arg_boo);
}
...
my_interface_skeleton_get_int64_property (MyInterface *object)
{
  ...
  gint64 value;
  g_mutex_lock (&skeleton->priv->lock);
  value = g_marshal_value_peek_int64 (&(skeleton->priv->properties[7]));
  g_mutex_unlock (&skeleton->priv->lock);
  return value;
}
...
my_interface_skeleton_get_array_uint32_property (MyInterface *object)
{
  ...
  GVariant *value;
  g_mutex_lock (&skeleton->priv->lock);
  value = g_marshal_value_peek_variant (&(skeleton->priv->properties[11]));
  g_mutex_unlock (&skeleton->priv->lock);
  return value;
}
  ...
  if (g_once_init_enter (&once_init_value))
    {
      ...
      g_hash_table_insert (lookup_hash, (gpointer) "com.example.MyInterface", (gpointer) (guintptr) (TYPE_MY_INTERFACE_PROXY));
